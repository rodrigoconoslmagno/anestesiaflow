...
export const PlantaoView = () => {
    ...
    const escalaSelecionada = useMemo(() => {
      ...
      const dataBusca = DateUtils.paraISO(date);
      
      return escalas.filter(escala => escala.data === dataBusca);
    }, [date, escalas]);
    ...
    const deleteItem = (medicoId: number, data: string, horaInicio: string) => {
      setEscalas(prevEscalas => {
        return prevEscalas.map(escala => {
          // Verifica se é a escala do médico e data corretos
          if (escala.medicoId === medicoId && escala.data === data) {
            // Remove os itens que pertencem ao bloco de horário clicado
            // (Considerando que o sistema salva blocos de horas subsequentes)
            const novosItens = escala.itensPlantao.filter(item => item.hora !== horaInicio);
          ...
          return escala;
        }).filter(escala => escala.itensPlantao.length > 0); // Remove a escala se ficar vazia
      });
    };
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 md:mb-6 gap-3 md:gap-4">
                     ...
                     {escalaSelecionada.length === 0 ? (
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                          {escalaSelecionada.map((escala, idx) => (
                             escala.itensPlantao?.map((item, idxItem) => {
                              const apenasNumeros = item.hora.replace(/\D/g, '');
                              const horaSlot = parseInt(apenasNumeros.substring(0, 2), 10);
                              if (horaSlot == 7 || horaSlot == 13 || horaSlot == 19) {
                                return (
                                <div key={idx || idxItem} className="relative bg-white p-3 rounded-xl border border-slate-100 hover:border-blue-200 transition-all shadow-sm flex items-center gap-3 group">
                                  <button
                                    onClick={() => deleteItem(escala.medicoId, escala.data, item.hora)}
                                  ...
                                  >
                                    ...
                                    >
                                        {item.icone ? (
                                            <img 
                                                  src={((item.icone as any) as string).startsWith('data:') 
